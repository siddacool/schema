...
export const SCHEMA_LOCAL_STORAGE = 'SCHEMA_LOCAL_STORAGE';
...
export function getLocalStore(storageType: StorageType): StorageValue {
  ...
  const storeRaw =
    storageType === 'local'
      ? localStorage.getItem(SCHEMA_LOCAL_STORAGE)
      : sessionStorage.getItem(SCHEMA_LOCAL_STORAGE);
...
export function setLocalStore(storageType: StorageType, data: StorageValue): StorageValue {
  ...
  if (storageType === 'local') {
    localStorage.setItem(SCHEMA_LOCAL_STORAGE, storeData);
  } else {
    sessionStorage.setItem(SCHEMA_LOCAL_STORAGE, storeData);
  }
